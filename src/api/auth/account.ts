...
export async function changePassword(payload: TChangePasswordPayload): Promise<void> {
    ...
    await axiosInstance.patch('/api/v1/members/passwords', body, { withCredentials: true });
}

// 닉네임 변경
export async function changeNickname(payload: TChangeNicknamePayload): Promise<TChangeNicknameResponse> {
    const { data } = await axiosInstance.patch<TChangeNicknameResponse>('/api/v1/members/infos', payload, { withCredentials: true });
    return data;
}
