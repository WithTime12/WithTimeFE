export const DateCourseQuestion = [
    {
        id: 0,
        filterTitle: '예산 범위',
        title: '오늘 데이트, 얼마나 쓰실 생각이세요?',
        subTitle: '(1인당)',
        keyword: '예산 범위',
        options: ['1만원 이하', '1-2만원', '2-3만원', '3만원 이상'],
        apiRequestValue: ['UNDER_10K', 'FROM_10K_TO_20K', 'FROM_20K_TO_30K', 'OVER_30K'],
        type: 'choice',
    },
    {
        id: 1,
        title: '어디에서 만날까요?',
        subTitle: null,
        keyword: '만날 장소',
        options: null,
        type: 'search',
        apiRequestValue: null,
        filterTitle: '만날 장소',
    },
    {
        id: 2,
        title: '얼마나 함께할 예정인가요?',
        subTitle: '(1개만 선택 가능)',
        keyword: '데이트 시간',
        options: ['1-2시간', '3-4시간', '반나절', '하루 종일'],
        apiRequestValue: ['ONETOTWO', 'THREETOFOUR', 'HALFDAY', 'ALLDAY'],
        type: 'choice',
        filterTitle: '데이트 시간',
    },
    {
        id: 3,
        title: '어떤 식사를 함께할 계획인가요?',
        subTitle: null,
        keyword: '식사 구성',
        options: ['아침', '점심', '저녁'],
        apiRequestValue: ['BREAKFAST', 'LUNCH', 'DINNER'],
        type: 'choices',
        filterTitle: '식사 구성',
    },
    {
        id: 4,
        title: '어떻게 이동하실 건가요?',
        subTitle: null,
        keyword: '이동 수단',
        options: ['도보', '자차', '대중교통'],
        apiRequestValue: ['WALK', 'CAR', 'PUBLICTRAN'],
        type: 'choice',
        filterTitle: '',
    },
    {
        id: 5,
        title: '오늘은 어떤 분위기의 데이트를 원하시나요?',
        subTitle: null,
        keyword: '데이트 키워드',
        options: null,
        apiRequestValue: null,
        type: 'keyword',
        filterTitle: '데이트 키워드',
    },
    {
        id: 6,
        title: '데이트 시작 시간을 알려주세요!',
        subTitle: '24:00 기준',
        keyword: null,
        options: null,
        apiRequestValue: null,
        type: 'time',
        filterTitle: '만날 시간',
    },
];

export enum Budget {
    UNDER_10K = '1만원 이하',
    FROM_10K_TO_20K = '1-2만원',
    FROM_20K_TO_30K = '2-3만원',
    OVER_30K = '3만원 이상',
}

export enum TotalTime {
    ONETOTWO = '1-2시간',
    TWOTOTHREE = '2-3시간',
    THREETOFOUR = '3-4시간',
    HALFDAY = '반나절',
    ALLDAY = '하루 종일',
}

export enum Meal {
    BREAKFAST = '아침',
    LUNCH = '점심',
    DINNER = '저녁',
}
